...
def build_graph(model: ChatOpenAI):
    ...
    # Node generator: yields partial state updates as tokens arrive.
...
st.set_page_config(page_title="Chat com PDF (streaming)", page_icon="📄")
st.title("Pergunte ao seu PDF")
st.caption("Respostas em tempo real (streaming) com LangGraph + Streamlit.")
...
if question:
    ...
    with st.chat_message("assistant"):
        inputs = {"context": st.session_state.pdf_text, "question": question, "answer": ""}

        def token_generator():
            last = ""
            # LangGraph can stream either full state ("values") or per-node updates (default).
            for update in app.stream(inputs, stream_mode="values"):
                current = update.get("answer") if isinstance(update, dict) else None
                if not current:
                    continue
                ...
                if delta:
                    yield delta
                last = current
            st.session_state._last_answer = last

        st.session_state._last_answer = ""
        try:
            ...
            st.session_state._last_answer = full

        answer = st.session_state._last_answer or ""
